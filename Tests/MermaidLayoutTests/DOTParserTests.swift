...
final class DOTParserTests: XCTestCase {
    ...
    func testRendersThroughASCIIRenderer() {
        let chart = parse(#"digraph { rankdir=LR; A[label="Start"]; A -> B -> C }"#)
        let out = ASCIIRenderer.asciiRenderFlowchart(chart, color: .plain)
        XCTAssertNotNil(out)
        XCTAssertTrue(out?.contains("Start") ?? false)
    }
}
